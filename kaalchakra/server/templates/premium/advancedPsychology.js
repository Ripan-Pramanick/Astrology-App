...
const generateAdvancedPsychology = (data) => {
    ...
    const psychData = data.psychologyAnalysis || {
        atmakaraka: "Sun", moonPosition: "Taurus (Exalted)", mercuryPosition: "Aries",
        emotionalLandscape: "With an exalted Moon, you possess a highly stable and emotionally grounded nature...",
        shadowSelf: "Your shadow self is influenced by the placement of Rahu and the 8th house...",
        ...
        <div class="content-wrapper">
            ...
            <div class="indicators-grid">
                <div class="indicator-card">
                    <span class="indicator-label">${t.atmakaraka || "Atmakaraka"}</span>
                    <span class="indicator-value">${psychData.atmakaraka}</span>
                    <div class="indicator-sub">${t.soulsCoreDesire || "Soul's Core Desire"}</div>
                </div>
                <div class="indicator-card">
                    <span class="indicator-label">${t.moonMind || "Moon (Mind)"}</span>
                    <span class="indicator-value">${psychData.moonPosition.split(' ')[0]}</span>
                    <div class="indicator-sub">${psychData.moonPosition.includes('Exalted') || psychData.moonPosition.includes('Own') ? (t.strongStable || 'Strong & Stable') : (t.emotionalFilter || 'Emotional Filter')}</div>
                </div>
                <div class="indicator-card">
                    <span class="indicator-label">${t.mercuryIntellect || "Mercury (Intellect)"}</span>
                    <span class="indicator-value">${psychData.mercuryPosition}</span>
                    <div class="indicator-sub">${t.cognitiveProcessing || "Cognitive Processing"}</div>
                </div>
            </div>

            <div class="analysis-section">
                <h3 class="analysis-heading">${t.emotionalLandscape || "Emotional Landscape"}</h3>
            ...
            <div class="shadow-box">
                <div class="shadow-heading">🌑 ${t.shadowSelfTriggers || "The Shadow Self & Triggers"}</div>
            ...
            <div style="display: flex; gap: 15px; margin-bottom: 20px;">
                <div class="tags-section" style="flex: 1; margin-bottom: 0;">
                    <h4 style="font-size: 13px; color: #15803d; text-transform: uppercase;">${t.cognitiveStrengths || "Cognitive Strengths"}</h4>
                    <div class="tags-container">
                        ${strengthsHtml}
                    </div>
                </div>
                <div class="tags-section" style="flex: 1; margin-bottom: 0;">
                    <h4 style="font-size: 13px; color: #b91c1c; text-transform: uppercase;">${t.mentalBlindSpots || "Mental Blind Spots"}</h4>
                    <div class="tags-container">
                        ${blindSpotsHtml}
                    </div>
                </div>
            ...
            <div class="coping-box">
                <strong>${t.recommendedGrounding || "Recommended Grounding Practice:"}</strong> ${psychData.copingMechanism}
            </div>
