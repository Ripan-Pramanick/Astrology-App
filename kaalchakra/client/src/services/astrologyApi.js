...
export const astrologyServices = {
  kundli: {
    async getGeoDetails(place) {
      try {
        console.log("📡 Calling API with place:", place);
        const response = await api.post('/astrology/geo_details', { place: typeof place === 'string' ? place : place.place });
        ...
        if (response.data && response.data.success && response.data.data) {
          ...
          if (Array.isArray(data)) {
            placesList = data.map(loc => ({
              place_name: loc.display_name || loc.place_name,
              lat: loc.lat || loc.latitude,
              lng: loc.lon || loc.longitude,
        ...
        console.log("📋 Processed places list:", placesList);
        return { geonames: placesList };
    ...
    async getBirthDetails(payload) {
      const response = await api.post('/astrology/birth_details', payload);
      return response.data.data;
    }
  },
  planetary: {
    async getPlanetsExtended(payload) {
      const response = await api.post('/astrology/planets', payload);
      return response.data.data;
    }
  },
  predictions: {
  ...
  panchang: {
    async getBasicPanchang(params) {
      const response = await api.post('/astrology/birth_details', params);
      return response.data.data;
    },
    async getAdvancedPanchang(params) {
      const response = await api.post('/astrology/birth_details', params);
  ...
  dosha: { async getYogas(params) { return []; } },
  dasha: { async getCurrentVDasha(params) { return { mahadasha: [] }; } }
};
