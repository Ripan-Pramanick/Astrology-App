...
const Horoscope = () => {
  ...
  const zodiacSigns = [
    { name: 'Aries', date: 'Mar 21 - Apr 19', image: Aries, icon: '♈', element: 'Fire', apiName: 'aries' },
    { name: 'Taurus', date: 'Apr 20 - May 20', image: Taurus, icon: '♉', element: 'Earth', apiName: 'taurus' },
    { name: 'Gemini', date: 'May 21 - Jun 20', image: Gemini, icon: '♊', element: 'Air', apiName: 'gemini' },
    { name: 'Cancer', date: 'Jun 21 - Jul 22', image: Cancer, icon: '♋', element: 'Water', apiName: 'cancer' },
    { name: 'Leo', date: 'Jul 23 - Aug 22', image: Leo, icon: '♌', element: 'Fire', apiName: 'leo' },
    { name: 'Virgo', date: 'Aug 23 - Sep 22', image: Virgo, icon: '♍', element: 'Earth', apiName: 'virgo' },
    { name: 'Libra', date: 'Sep 23 - Oct 22', image: Libra, icon: '♎', element: 'Air', apiName: 'libra' },
    { name: 'Scorpio', date: 'Oct 23 - Nov 21', image: Scorpio, icon: '♏', element: 'Water', apiName: 'scorpio' },
    { name: 'Sagittarius', date: 'Nov 22 - Dec 21', image: Sagittarius, icon: '♐', element: 'Fire', apiName: 'sagittarius' },
    { name: 'Capricorn', date: 'Dec 22 - Jan 19', image: Capricorn, icon: '♑', element: 'Earth', apiName: 'capricorn' },
    { name: 'Aquarius', date: 'Jan 20 - Feb 18', image: Aquarius, icon: '♒', element: 'Air', apiName: 'aquarius' },
    { name: 'Pisces', date: 'Feb 19 - Mar 20', image: Pisces, icon: '♓', element: 'Water', apiName: 'pisces' }
  ];
  ...
  // ... (getPlanetaryInfluence, getLovePrediction, getCareerPrediction, getHealthPrediction, getLuckyNumber, getLuckyColor functions remain the same as they handle fallback business logic)
  ...
  const getLuckyNumber = (sign) => {
    const numbers = { 'Aries': 3, 'Taurus': 6, 'Gemini': 5, 'Cancer': 2, 'Leo': 1, 'Virgo': 7, 'Libra': 4, 'Scorpio': 9, 'Sagittarius': 8, 'Capricorn': 10, 'Aquarius': 11, 'Pisces': 12 };
    return numbers[sign] || 7;
  };

  const getLuckyColor = (sign) => {
    const colors = { 'Aries': 'Crimson Red', 'Taurus': 'Forest Green', 'Gemini': 'Golden Yellow', 'Cancer': 'Ocean Blue', 'Leo': 'Royal Purple', 'Virgo': 'Emerald', 'Libra': 'Rose Gold', 'Scorpio': 'Crimson Red', 'Sagittarius': 'Sapphire Blue', 'Capricorn': 'Forest Green', 'Aquarius': 'Ocean Blue', 'Pisces': 'Royal Purple' };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 mb-10">
          <div className="flex items-center justify-center gap-2 mb-6">
            ...
            <h2 className="text-xl font-semibold text-gray-800">{t('horoscope.chooseSign')}</h2>
            <span className="text-2xl">✨</span>
          </div>
              ...
              >
                <div className="h-16 flex items-center justify-center mb-2">
                  {!imageErrors[sign.name] ? (
                    <img
                      src={sign.image}
                      alt={t(`horoscope.zodiac.${sign.name}`, sign.name)}
                ...
                <div className="flex flex-col items-center justify-center w-full mt-1">
                  <span className={`text-sm md:text-base font-bold transition-colors ${
                    ...
                    {t(`horoscope.zodiac.${sign.name}`, sign.name)}
                  </span>
          ...
          <div className="p-6 md:p-8 lg:p-10">
              ...
              <div className="text-center sm:text-left">
                <div className="flex items-center gap-2 justify-center sm:justify-start">
                  <h2 className="text-2xl md:text-3xl font-bold text-gray-800">
                    {t('horoscope.signHoroscope').replace('{{sign}}', t(`horoscope.zodiac.${selectedSign}`))}
                  </h2>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${getElementBg(element)} text-gray-600 font-medium`}>
                    {t(`horoscope.elements.${element}`, { defaultValue: element })}
                  </span>
                </div>
                <p className="text-orange-500 font-medium mt-1">
                  {t('horoscope.celestialGuidance').replace('{{day}}', t(`horoscope.days.${selectedDay}`))}
                </p>
            ...
            <div className="mb-8 p-5 bg-gradient-to-r from-orange-50 to-amber-50 rounded-xl">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-bold text-gray-800">{t('horoscope.general')}</h3>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
              ...
              <div className="bg-gradient-to-br from-pink-50 to-rose-50 rounded-xl p-5 border border-pink-100 hover:shadow-md transition-shadow">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-bold text-gray-800">{t('horoscope.love')}</h3>
                </div>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-5 border border-blue-100 hover:shadow-md transition-shadow">
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-2xl">💼</span>
                  <h3 className="font-bold text-gray-800">{t('horoscope.career')}</h3>
                </div>
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-5 border border-emerald-100 hover:shadow-md transition-shadow">
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-2xl">🧘‍♀️</span>
                  <h3 className="font-bold text-gray-800">{t('horoscope.health')}</h3>
                </div>
            ...
            <div className="flex flex-wrap justify-center gap-4 pt-6 border-t border-gray-100">
              <div className="flex items-center gap-3 bg-gray-50 px-5 py-2.5 rounded-full border border-gray-200">
                <span className="text-gray-500 font-medium text-sm">{t('horoscope.luckyNumber')}</span>
                <span className="text-orange-600 font-bold text-xl">{horoscope.lucky_number}</span>
              </div>
              <div className="flex items-center gap-3 bg-gray-50 px-5 py-2.5 rounded-full border border-gray-200">
                <span className="text-gray-500 font-medium text-sm">{t('horoscope.luckyColor')}</span>
              ...
              <div className="flex items-center gap-3 bg-gray-50 px-5 py-2.5 rounded-full border border-gray-200">
                <span className="text-gray-500 font-medium text-sm">{t('horoscope.elementLabel')}</span>
                <span className="text-orange-600 font-semibold text-md">{t(`horoscope.elements.${element}`, { defaultValue: element })}</span>
              </div>
            </div>
